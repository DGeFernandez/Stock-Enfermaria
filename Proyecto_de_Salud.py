...
def eliminar_insumo():
    nombre = input("Ingrese el nombre del insumo que desea eliminar: ")
    conn = sqlite3.connect('centro_medico.db')
    c = conn.cursor()
    c.execute('DELETE FROM insumos WHERE nombre = ?', (nombre,))
    conn.commit()
    conn.close()
    print(f"Insumo '{nombre}' eliminado con éxito.")
